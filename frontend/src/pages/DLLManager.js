...
import { FaFolder, FaSync, FaChevronDown, FaChevronUp, FaServer, FaBalanceScale, FaTimes, FaUpload, FaSpinner, FaExclamationTriangle, FaCheckSquare, FaSquare } from 'react-icons/fa';
...
const DLLManager = () => {
  ...
  const [updateTargetServers, setUpdateTargetServers] = useState([]); // Changed to array for multi-select
  ...
  const openUpdateModal = () => {
    ...
    setUpdateTargetServers([]); // Reset to empty array
    setUpdateDllName('');
    setUpdateVersion('');
  ...
  const toggleTargetServer = (serverName) => {
    setUpdateTargetServers(prev => {
  ...
  // Select/Deselect all target servers
  const toggleAllTargetServers = () => {
    const availableTargets = allDLLs
      .filter(s => s.serverName !== updateSourceServer)
      .map(s => s.serverName);
    
    if (updateTargetServers.length === availableTargets.length) {
      setUpdateTargetServers([]);
    } else {
      setUpdateTargetServers(availableTargets);
    }
  };
  ...
  const handleUpdateConfirm = async () => {
    ...
    try {
      const response = await dllAPI.update(
        updateSourceServer,
        updateTargetServers, // Send array of servers
        updateDllName,
        updateVersion
      );
      ...
      setUpdateResult({
        ...
        results: response.data.results // Array of results for each server
      });
  ...
  return (
    <div className="space-y-6">
      ...
      {/* UPDATE DLL MODAL - WITH MULTI-SELECT */}
      {showUpdateModal && (
            ...
            <div className="p-6 space-y-5">
              {!updateResult && (
                <>
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setUpdateTargetServers([]); // Reset target servers when source changes
                        setUpdateDllName('');
                        setUpdateVersion('');
                      }}
                      disabled={updating}
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                        Target Servers * (Select one or more)
                      </label>
                      {updateSourceServer && availableTargetServers.length > 0 && (
                        <button
                          onClick={toggleAllTargetServers}
                          disabled={updating}
                        ...
                        >
                          {updateTargetServers.length === availableTargetServers.length ? 'Deselect All' : 'Select All'}
                        </button>
                      )}
                    </div>
                    
                    {!updateSourceServer ? (
                      <div className="bg-gray-100 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg p-4 text-center">
                        <p className="text-gray-500 dark:text-gray-400 text-sm">
                          Please select a source server first
                        </p>
                      </div>
                    ) : availableTargetServers.length === 0 ? (
                      ...
                      <div className="bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg max-h-64 overflow-y-auto">
                        {availableTargetServers.map(server => (
                          <label
                            key={server.serverName}
                            className={`flex items-center space-x-3 p-3 hover:bg-gray-50 dark:hover:bg-gray-600 cursor-pointer border-b border-gray-200 dark:border-gray-600 last:border-b-0 transition-colors ${
                              updateTargetServers.includes(server.serverName) ? 'bg-green-50 dark:bg-green-900/20' : ''
                            }`}
                          >
                            <input
                              type="checkbox"
                              checked={updateTargetServers.includes(server.serverName)}
                              onChange={() => toggleTargetServer(server.serverName)}
                              disabled={updating}
                            ...
                            <div className="flex items-center space-x-2 flex-1">
                              {updateTargetServers.includes(server.serverName) ? (
                    ...
                    {updateTargetServers.length > 0 && (
                      <div className="mt-2 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-lg p-3">
                        <p className="text-sm font-medium text-green-800 dark:text-green-300">
                          {updateTargetServers.length} server{updateTargetServers.length !== 1 ? 's' : ''} selected
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  {updateSourceServer && updateTargetServers.length > 0 && updateDllName && updateVersion && !updating && (
                    <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-lg p-4">
                      ...
                      <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                        ...
                        <li>• <strong>To:</strong> {updateTargetServers.length} server{updateTargetServers.length !== 1 ? 's' : ''}</li>
                        <li className="ml-4 text-xs">
                          {updateTargetServers.map((server, idx) => {
                            const serverInfo = allDLLs.find(s => s.serverName === server);
                            return (
                              <div key={server}>
                                {idx + 1}. {serverInfo?.serverGroup} ({server})
                              </div>
                            );
                          })}
                        </li>
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-gray-900 dark:text-white">
                            Updating DLL on {updateTargetServers.length} server{updateTargetServers.length !== 1 ? 's' : ''}...
                          </p>
              ...
              {updateResult && (
                <div className={`rounded-lg p-6 border-2 ${
                  ...
                  {updateResult.success && updateResult.results && updateResult.results.length > 0 && (
                    <div className="space-y-3">
                      ...
                      {updateResult.results.map((result, idx) => (
                        <div 
                          key={idx}
                          className={`bg-white dark:bg-gray-800 rounded p-4 border-l-4 ${
                            result.success 
                              ? 'border-green-500' 
                              : 'border-red-500'
                          }`}
                        >
                          ...
                          {result.success ? (
                            <div className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                              <p>✓ Main deployment completed</p>
                              {result.data?.deploymentResults?.additionalPaths && result.data.deploymentResults.additionalPaths.length > 0 && (
                                <p>✓ {result.data.deploymentResults.additionalPaths.filter(p => p.success).length}/{result.data.deploymentResults.additionalPaths.length} additional paths</p>
                              )}
                              {result.data?.deploymentResults?.frontendServers && result.data.deploymentResults.frontendServers.length > 0 && (
                                <p>✓ {result.data.deploymentResults.frontendServers.filter(f => f.success).length}/{result.data.deploymentResults.frontendServers.length} frontend servers</p>
                              )}
                              {result.data?.deploymentResults?.database && (
                                <p>{result.data.deploymentResults.database.success ? '✓' : '✗'} Database update</p>
                              )}
                            </div>
                          ) : (
                ...
                {!updateResult ? (
                  <>
                    ...
                    <button
                      ...
                      disabled={!updateSourceServer || updateTargetServers.length === 0 || !updateDllName || !updateVersion || updating}
                    ...
                    >
                      ...
                      <span>
                        {updating 
                          ? `Deploying to ${updateTargetServers.length} server${updateTargetServers.length !== 1 ? 's' : ''}...` 
                          : `Deploy to ${updateTargetServers.length} Server${updateTargetServers.length !== 1 ? 's' : ''}`
                        }
                      </span>
                    </button>
                  </>
